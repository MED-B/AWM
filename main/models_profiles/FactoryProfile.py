...
class FactoryAdmin(admin.ModelAdmin): 
    list_display = ['vendor_FirstName','vendor_LastName','factoryaddress','factorysize']
    ...
    list_filter = ['vendorid__companyid__userid__firstname',
                   'vendorid__companyid__userid__lastname',
                   'factoryaddress','factorysize']
    
    def response_based_on_group(self,request):
         # Limit change permission based on the user's group
            if request.user.groups.filter(name='FactoryAdminGroup').exists() or request.user.groups.filter(name='AllAdminGroup').exists():
